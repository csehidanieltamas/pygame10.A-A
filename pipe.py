...
class Pipe:
    ...
    def __init__(self, x: int) -> None:
        self._y: int = random.randint(500, 700) # véletlenszerű helyt adunk a két cső közötti lyuknak
        self._player_img: pygame.Surface = pygame.image.load(
            "imageSource/Brick Pipe.png"
        ).convert_alpha() # cső képének betöltése
        self._rect: pygame.Rect = self._player_img.get_rect(topleft=(x, self._y)) # cső hitboxa
    ...
    def draw(self, win: pygame.Surface) -> None: # csövek rajzolása
        win.blit(pygame.transform.flip(self._player_img, False, True), self._toprect) # felső cső
